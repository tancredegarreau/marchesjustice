...
const Global = createGlobalStyle<{ color: string; filter: string }>`
...
export const MapMarker: FC<Props> = ({ colorHex, ...props }) => {
  ...
  const [markerIcon] = useState(
    new Icon({
      iconUrl: require("./marker.svg").default,
      iconRetinaUrl: require("./marker.svg").default,
      iconSize: [40, 50],
      iconAnchor: [20, 50],
      popupAnchor: [0, -40],
      className: `marker-${colorHex.slice(1)}`,
    })
  );

  return (
    <>
      <Global color={colorHex} filter={filter} />
      <Marker icon={markerIcon} {...props} />
    </>
  );
};
